...
Bits are any kind of two-valued things, usually
...
{{figure {url: "img/bit-sea.png", alt: "The Ocean of Bits"}}}
...
To create a value, you must merely invoke its
...
Fractional numbers are written by using a
dot.
...
Calculations with whole numbers (also called _((integer))s_)
...
There are three special values in
...
{{index syntax, text, character, [string, notation], "single-quote character", "double-quote character", "quotation mark"}}
...
when we get to methods in [Chapter 4](04_data.html#methods).
...
The `>` and `<` signs are the traditional
...
_Not_ is written as an exclamation mark
...
The last logical operator I will discuss is not unary, not
...
When comparing values of the same type using `==`, the
...
it's an expression that does something _terrible_—the result will be
...
[next chapter](02_program_structure.html#program_structure) will
