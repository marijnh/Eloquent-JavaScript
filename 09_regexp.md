...
In this chapter, I will discuss one such
...
Within square brackets, a dash (`-`) between two
...
That looks completely awful, doesn't it? It has way too
...
[later](09_regexp.html#date_regexp_counted).
...
We now know how to match a single digit. What
...
The first and second `+` characters apply only to the
...
The `i` at the end of the expression in the
...
You can also create an object for a specific
time.
...
Putting ((parentheses)) around the parts of the
...
Note that a boundary marker doesn't represent an actual
...
Say we want to know whether a piece of text contains not
...
Parentheses can be used to limit the part of the
...
Our expression matches a string if we can find a path
...
So the matcher _backtracks_. When
...
The matcher stops as soon as it finds a full
...
It isn't hard to use `replace` to
...
There are cases where you might not know the exact
...
The `indexOf` method on strings cannot be
...
The `exec` method similarly
does not provide a convenient way to start searching from a given
position in the string. But it does provide an _in_convenient way.
...
explicitly use _lastIndex_—are typically the only places where you
want to use them.
...
do this in [Chapter 6](06_object.html#split). Some operating
...
By a strange historical accident, `\s` (whitespace)
...
It is almost unavoidable that, in the course
...
[chapter summary](09_regexp.html#summary_regexp) for help. Test each
...
A series of ((digit))s can be matched by the simple
...
the number, the decimal dot, and exponent notation—`5e-3` or _1E10_—
...
Finally, to make the _e_ case-insensitive, either
