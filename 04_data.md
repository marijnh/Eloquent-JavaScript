...
Numbers, Booleans, and strings are the
bricks that ((data)) structures are built from. But you can't make
much of a house out of a single brick. _Objects_ allow us to group
...
Switching to a more scientific approach, Jacques intends
...
that trigger the transformations.
...
To work with a chunk of digital data, we'll first
...
5 7 11"` as our representation. But this is awkward. You'd have to
...
is called an _array_ and is written as a list of values between
...
expression that corresponds to the _((index))_ given by the expression
in the brackets.
...
value. In the first case, we access the `length` property of the value
in `myString`. In the second, we access the property named `max` in
the `Math` object (which is a collection of mathematics-related values
and functions).
...
`value.x` and `value[x]` access a ((property)) on _value_—but not
necessarily the same property. The difference is in how `x` is
...
{{index array, ["length property", "for array"], [array, "length of"]}}
...
them. The `length` property of an array tells us how many elements it
...
write `array.length` because that is easier to write than
`array["length"]`.
...
Both string and
array objects contain, in addition to the `length` property, a number
...
Every string has a `toUpperCase` property. When called, it
will return a copy of the string, in which all letters have been
converted to uppercase. There is also `toLowerCase`. You can guess
what that does.

{{index this}}

Interestingly, even though the call to `toUpperCase` does
...
described in [Chapter 6](06_object.html#obj_methods).
...
{{index collection, array, string, "push method", "pop method", "join method"}}
...
Back to the weresquirrel. A set of daily log
...
{{index syntax, object, property, "curly braces", "{} (object)"}}

Values of the type _object_ are arbitrary collections of
...
_immutable_—it is impossible to change an existing value of those
...
{{index [object, identity], identitiy, memory}}
...
{{index "tentacle (analogy)", [variable, "model of"]}}
...
JavaScript's `==` operator, when comparing objects, will
...
_Correlation_ is a measure of ((dependence)) between
...
For binary (Boolean) variables, the _phi_
...
The body of the loop in `tableFor` figures
...
A _((map))_ is a way to go from values in one
...
we will discuss in [Chapter 6](06_object.html#prototypes), but for
...
Most correlations seem to lie close to zero. Eating
...
All is well with Jacques for a while. But a
...
Before finishing up this chapter,
...
We can read properties like `length` and
...
{{index ["length property", "for string"], "charAt method", [string, indexing]}}
...
{{index "arguments object", "length property", parameter, "optional argument", "array-like object"}}
...
{{index journal, "console.log", "variadic function"}}
...
{{index ["arguments object", indexing]}}
...
[Chapter 13](13_dom.html#sin_cos), I'll explain them.
...
{{index "global object", "window variable", "global scope", scope, object}}
...
Having `range` understand negative
...
Arrays have a method `reverse`, which
...
[previous chapter](03_functions.html#pure), which variant do you
...
To run over a list (in `listToArray` and `nth`), a `for`
...
The recursive version of `nth` will, similarly, look at
...
head node. To get element _N_ + 1, you take the _N_th element of the
...
The `==` operator compares objects by
...
Use a `for`/`in` loop to go over the
...
Returning the correct value from the function is
