{{meta {chap_num: 8, prev_link: 07_elife, next_link: 09_regexp, load_files: ["code/chapter/08_error.js"]}}}
...
Flaws in a program are usually called ((bug))s.
...
When it comes to programmer mistakes, our
...
{{index "programming language", type, "static typing", "dynamic typing", "run-time error", error}}
...
The process of finding mistakes—bugs—in programs is
called _debugging_.
...
{{index "var keyword", [variable, global]}}
...
So the bogus call to `Person` succeeded but returned an
...
{{index parameter, [variable, naming], "with statement"}}
...
In short, putting a `"use strict"` at the top of your
program rarely hurts and might help you spot a problem.

## Testing

{{index "test suite", "run-time error"}}

If the language is not going to do
...
{{index "test suite", "testing framework", "domain-specific language"}}

Writing tests like this tends to produce rather repetitive,
...
Once you notice that there is something wrong with your
...
But not always. Sometimes the line that triggered
...
Even if you see the problem already, pretend for a
moment that you don't. We know that our program is malfunctioning, and
we want to find out why.

{{index "trial and error"}}

This is where you must resist the urge to start
...
Simple programs, or programs that run only under
...
In many situations, mostly when ((error))s are
...
Exceptions are a mechanism that make it possible for code that
...
calls that was mentioned in [Chapter 3](03_functions.html#stack).
...
The `throw` keyword is used to raise an
...
{{index debugging, "call stack", "Error type", "stack trace"}}

In this case, we used the `Error` ((constructor)) to create
...
Note that the function `look` completely
...
{{index "exception handling", "cleaning up", "withContext function", "dynamic scope"}}
...
For programmer mistakes or problems
...
For problems that are _expected_ to happen during
...
When a `catch` body is entered, all we know is that
...
So we want to catch a _specific_ kind of
...
{{index "Error type", "instanceof operator"}}
...
The assignment to the `stack` property tries to give
...
This will catch only instances of `InputError` and let
...
{{index assertion, debugging}}

Assertions are a way to make sure
...
The call to `primitiveMultiply` should
...
call succeeds—as in the [`look` example](08_error.html#look)
