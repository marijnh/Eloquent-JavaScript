...
You've seen function values, such
...
The most obvious application of functions is
...
Typical adult English speakers have some 20,000 words
...
{{index square, [function, definition]}}

A function definition is just a
...
A function can have multiple parameters or no
...
{{index [function, application], [variable, "from parameter"]}}
...
{{index [function, scope], scope, "local variable"}}
...
{{index "var keyword", "global scope", [variable, global]}}
...
The following code demonstrates this. It
...
{{index [variable, naming], scope, "global scope", [code, "structure of"]}}
...
JavaScript distinguishes not just between _global_ and
...
For example, this rather nonsensical function
...
The `flat` and `mountain` functions
...
{{index "let keyword", "ECMAScript 6"}}

If you find this odd, you're not
...
Function ((variable))s usually simply act as
...
But the two are different. A function value
...
In
[Chapter 5](05_higher_order.html#higher_order), we will discuss the
wonderful things that can be done by passing around function values to
other functions.

## Declaration notation

{{index syntax, "square example", "function keyword", [function, definition], [function, declaration]}}

There is
a slightly shorter way to say “_var square = function…_”. The
...
This is a function _declaration_. The
...
What happens when you put such a function
...
Because a function has to jump back to
...
{{index "power example", "optional argument"}}
...
In the link:04_data.html#arguments_object[next
...
{{index "call stack", "local variable", [function, "as value"], closure, scope}}
...
{{index [variable, "from parameter"]}}

The explicit `localVariable` from the
`wrapValue` example isn't needed since a parameter is itself a local
variable.

{{index [function, "model of"]}}

Thinking about programs like this takes some
...
The dilemma of speed versus ((elegance)) is an
...
The basic rule, which has been repeated by many
...
The reason I'm stressing this is that
...
But recursion is not always just a
...
To better understand how this function produces the
...
There are two more or less natural ways for
functions to be introduced into programs.

{{index repetition}}

The first is that you find yourself writing very
...
{{index [function, naming], [variable, naming]}}
...
We sure can. But just as we're in the
...
Instead of lifting out the repeated part of our
...
A function with a nice, obvious
...
A _pure_ function is a specific kind of
...
The
[previous chapter](02_program_structure.html#return_values)
...
[example](03_functions.html#recursive_puzzle) in this chapter, with
...
A ((loop)) in your function will have
...
Take care to make all the variables used in the
