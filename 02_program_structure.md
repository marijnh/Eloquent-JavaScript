...
In
[Chapter 1](01_values.html#values), we made some values and then
...
A fragment of code that produces a value is
...
In some cases, JavaScript allows you to
...
{{index syntax, [variable, definition], "side effect", memory}}
...
{{index "underscore character", "dollar sign", [variable, naming]}}

Variable names can be any word that isn't
...
{{index "= operator", assignment, [variable, assignment]}}
...
{{index [variable, "model of"], "tentacle (analogy)"}}
...
{{figure {url: "img/octopus.jpg", alt: "Variables as tentacles"}}}
...
A single `var` statement may define multiple
...
{{index syntax, "implements (reserved word)", "interface (reserved word)", "let keyword", "package (reserved word)", "private (reserved word)", "protected (reserved word)", "public (reserved word)", "static (reserved word)", "void operator", "yield (reserved word)", "reserved word", [variable, naming]}}
...
{{index output, function, [function, application], "alert function", "message box"}}
...
A lot of the values provided in the
...
{{figure {url: "img/alert.png", alt: "An alert dialog",width: "8cm"}}}

{{index parameter, [function, application]}}

Executing a function is
...
Though ((variable)) names cannot contain ((period
...
[Chapter 4](04_data.html#properties).
...
Showing a dialog box or writing text to
...
When a function produces a value, it is said to _return_
...
{{index "dialog box", input, browser, "confirm function"}}
...
{{figure {url: "img/confirm.png", alt: "A confirm dialog",width: "8cm"}}}

{{index input, "prompt function", "text input"}}
...
The function `Number` converts a
...
Executing statements in straight-line
...
The `isNaN` function is a standard JavaScript
...
If we have more than two paths to choose
...
Consider a program that prints all even numbers from
...
That works, but the idea of writing a program is to
...
Looping control flow allows us to go
...
{{index grouping, "{} (block)", block}}

In this loop, we want to both
...
Many JavaScript programmers wrap every single
...
The variable `number` demonstrates the way
...
As an example that actually does something useful,
...
[Chapter 4](04_data.html#array_indexing), it is a good idea to get
...
{{index block, "code structure", whitespace, "programming style"}}
...
Because this pattern is so common, JavaScript and
...
Here is the code that computes 2^10^, using `for`
instead of `while`:
...
Note that even though no block
...
The `for` construct in the example does not have a part
...
The `continue` keyword is similar to `break`, in
...
You may put any number of `case` labels
...
{{index capitalization, [variable, naming], whitespace}}
...
[Chapter 6](06_object.html#constructors). For now, the important
...
Often, raw code does not convey all the information
...
A `//` comment goes only to the end of the line. A
...
If you are unsure how to try your solutions to
exercises, refer to the [introduction](00_intro.html#intro).
...
You can start with a program that simply
prints out the numbers 1 to 7, which you can derive by making a few
modifications to the
[even number printing example](02_program_structure.html#loops)
...
The string can be built by starting with
...
To work with two ((dimensions)), you will need a
